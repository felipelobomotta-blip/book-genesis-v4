...
MENU: List[Preset] = [
    ...
    # The free second family: a Google account gives an AI Studio key in a minute, no card, and the
    # Flash models are free of charge on that API (pricing page, 2026-09-04). Verified with a real call.
    Preset("gemini-api", "Gemini API (free key from Google AI Studio, no card; Flash models are free)", "openai", "https://generativelanguage.googleapis.com/v1beta/openai", "GEMINI_API_KEY", "gemini-3.8-flash", "gemini-3.8-flash"),
...
# One family writing and judging: the judge is never the very model that wrote the prose when a
# sibling exists. Claude gets it from the preset (Opus writes, Sonnet judges); GPT needs this map
# (gpt-5.4 was the other id the probed Codex accepted). Quick start falls back if the sibling is refused.
SIBLING_JUDGE = {"codex": "gpt-5.4", "openai": "gpt-5.4"}


def judge_model_within(key: str, writer_model: str) -> str:
    preset = BY_KEY.get(key)
    if preset is None:
        return writer_model
    if preset.judge_model and preset.judge_model != writer_model:
        return preset.judge_model
    return SIBLING_JUDGE.get(key, writer_model)
...
def _quick(
    ...
    say("")
    writer = _entry_for(plan.writer.key, "writing", providers, ask, secret, say, None, None, interactive=False, from_env=_from_env(plan.writer))
    if plan.single_family:
        judge = (writer[0], judge_model_within(writer[0], writer[1]))
    else:
        judge = _entry_for(plan.judge.key, "judging", providers, ask, secret, say, None, None, interactive=False, from_env=_from_env(plan.judge))

    say(f"Checking writer: {_label(writer)} ...")
    ok, message = check(writer[0], writer[1], providers.get(writer[0]))
    if not ok:
        return _failed(say, message)
    say(f"  ok, {message}")

    say(f"Checking judge: {_label(judge)} ...")
    ok, message = check(judge[0], judge[1], providers.get(judge[0]))
    if not ok and plan.single_family and judge[1] != writer[1]:
        say(f"  {judge[1]} did not answer here ({message}); the judge falls back to {writer[1]}.")
        judge = (judge[0], writer[1])
        ok, message = check(judge[0], judge[1], providers.get(judge[0]))
    if not ok:
        return _failed(say, message)
    say(f"  ok, {message}")
    ...
    if plan.single_family:
        _single_family_note(say, writer, judge)
    return _save(providers, writer, judge, say, target)


def _from_env(detection: Detection) -> bool:
    return detection.detail.startswith("key in ")


def _label(entry: Tuple[str, str]) -> str:
    adapter_name, model = entry
    return f"{adapter_name}{' ' + model if model else ''}"


def _failed(say: Say, message: str) -> None:
    say(f"  failed: {message}")
    say("Nothing was saved. Fix the provider and run `book-genesis setup` again, or choose Custom.")
    return None


def _single_family_note(say: Say, writer: Tuple[str, str], judge: Tuple[str, str]) -> None:
    """Said whenever one provider both writes and judges. Honest about the limit, and about the
    one-minute fix: the Gemini API's Flash models are free, so a Google account is a second family."""
    say("Note: single family. The same provider writes and judges.")
    if judge[1] and judge[1] != writer[1]:
        say(f"  The judge runs on a different model ({judge[1]} reads what {writer[1]} wrote), blind and comparing")
        say("  drafts, but it shares the family's taste.")
    else:
        say("  The judge is the very model that wrote the prose; it will trust itself too much.")
    say("  A free second family takes a minute: a Gemini API key from Google AI Studio (Google account, no card,")
    say("  Flash models are free). Then `book-genesis setup` -> Change -> Custom -> judge = Gemini API.")
...
def _custom(
    ...
    if judge_index == len(judge_labels):
        default_model = judge_model_within(writer[0], writer[1])
    ...
    *,
    ...
    if key not in providers:
        ...
        if not preset.needs_key:
            ...
            entry["api_key_env"] = preset.key_env  # detected in the environment: nothing to ask
        else:
...
__all__ = ["run_setup", "MENU", "BY_KEY", "Preset", "text_choose", "Choose", "tag_model", "ROLE_GUIDANCE", "CLI_CANDIDATES", "judge_model_within"]
