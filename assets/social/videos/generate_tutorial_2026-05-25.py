...
T={
 'en':{
  ...
  'i_sub':"From an idea to a reviewable book project — step by step.",
  's1_lab':"STEP 1 · INSTALL",'s1_cap':"Clone the repo and install the runner.",
  's1':[('cmd','git clone .../book-genesis-v4'),('cmd','cd book-genesis-v4'),
        ('cmd','pip install -e .'),('ok','book-genesis command installed')],
  's2_lab':"STEP 2 · POINT ANY AGENT",'s2_cap':"Works in any agent that reads files.",
  's2':[('comment','# Claude Code / Codex / Antigravity / Kimi'),
        ('cmd','book-genesis setup'),
        ('out','providers and model roles configured'),
        ('out','runner workflow - saved project state - model feedback')],
  's3_lab':"STEP 3 · START A BOOK",'s3_cap':"One command. One idea. That's the whole input.",
  's3':[('cmd','book-genesis new --idea "an archivist'),
        ('cont','  who finds every deleted manuscript'),
        ('cont','  is still being edited somewhere"'),
        ('out','project created -> PROJECT_STATE.yaml')],
  's4_lab':"STEP 4 · THE RUNNER WORKS",'s4_cap':"Each step saves an artifact the next step can use.",
  's4':[('out','running pipeline...'),
        ...
        ('accent','blind model reading  (after drafting)'),
        ('ok','revision          respond to reader flags'),
        ('ok','review            inspect report + artifacts'),
        ('ok','export            markdown or EPUB')],
  's5_lab':"STEP 5 · REVIEW THE SIGNALS",'s5_cap':"Model feedback guides revision; people decide publication.",
  's5':[('out','blind model readers report turn-page signals'),
        ('out','the runner keeps drafts, verdicts, and warnings'),
        ('out','human readers and editors remain essential'),
        ('blank',''),
        ('accent','review report   inspect artifacts')],
  's6_lab':"STEP 6 · WHAT YOU GET",'s6_cap':"Every quality claim is backed by a file you can open.",
  's6':[('ok','manuscript/       full draft'),
        ('ok','artifacts/        brief to report'),
 ...
 'pt':{
  ...
  'i_sub':"De uma ideia a um projeto de livro revisável — passo a passo.",
  's1_lab':"PASSO 1 · INSTALAR",'s1_cap':"Clone o repo e instale o runner.",
  's1':[('cmd','git clone .../book-genesis-v4'),('cmd','cd book-genesis-v4'),
        ('cmd','pip install -e .'),('ok','comando book-genesis instalado')],
  's2_lab':"PASSO 2 · APONTE QUALQUER AGENTE",'s2_cap':"Funciona em qualquer agente que le arquivo.",
  's2':[('comment','# Claude Code / Codex / Antigravity / Kimi'),
        ('cmd','book-genesis setup'),
        ('out','provedores e modelos configurados'),
        ('out','workflow do runner - estado salvo - feedback de modelos')],
  's3_lab':"PASSO 3 · COMECE UM LIVRO",'s3_cap':"Um comando. Uma ideia. E so isso de entrada.",
  's3':[('cmd','book-genesis new --idea "um arquivista'),
        ('cont','  que descobre que todo manuscrito'),
        ('cont','  apagado ainda esta sendo editado"'),
        ('out','projeto criado -> PROJECT_STATE.yaml')],
  's4_lab':"PASSO 4 · O RUNNER TRABALHA",'s4_cap':"Cada etapa salva um artefato que a próxima pode usar.",
  's4':[('out','rodando pipeline...'),
        ...
        ('accent','leitura cega por modelos  (após o draft)'),
        ('ok','revisão           responde aos sinais do leitor'),
        ('ok','review            inspeciona relatório + artefatos'),
        ('ok','export            markdown ou EPUB')],
  's5_lab':"PASSO 5 · REVISE OS SINAIS",'s5_cap':"Feedback de modelos orienta a revisão; pessoas decidem a publicação.",
  's5':[('out','leitores-modelo cegos registram sinais de leitura'),
        ('out','o runner guarda drafts, vereditos e alertas'),
        ('out','leitores e editores humanos continuam essenciais'),
        ('blank',''),
        ('accent','review report   inspecione artefatos')],
  ...
  's6':[('ok','manuscript/       draft completo'),
        ('ok','artifacts/        do brief ao relatório'),
...
def sc_s5(d,t,dur): scene_step(d,t,dur,'s5_lab','s5_cap','s5',"book-genesis — review")
